...
def generate_code(account_id=None, parent=False, code="", company=None):
    """
    ...
    if not parent:
        print(company);
        last_code = Account.objects.filter(parent=None, company=company).last()
        if last_code:
...
def account_pre_save_receiver(sender, instance, *args, **kwargs):
    ...
    if not instance.id:
        if not instance.parent:
            instance.code=generate_code(None, False, "", instance.company)
        else:
